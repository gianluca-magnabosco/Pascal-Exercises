...
interface //serve para criar as variaveis globais e indicar os modulos que fazem parte da UNIT

type  								//cria novos tipos de informa�ao
      ...
      car:INTEGER;                 //             32
		 ...
		 cc=record 			//descri��o de registro para ser usada no vetor
      pf:integer;      //--                32
      cc:string[35];   //35*8 --          280
     end;              //tamanho ==>      312
     
     vetor=array[1..2000] of cc; 	// tipo vetor vetor de 2000 posi��es com duas informa�oes (integer e string[35]) por posi��o
     tipo=string[35];      				// tipo tipo 35 caracteres	
     a35=string[35];              // tipo a35 35 caracteres
		 vet1=array[1..2000] of a35;  // tipo vet1 vetor de 2000 posi��es com uma informa��o, (string[35]) por posi��o
...
{ordem eh um procedimento que ordena um vetor com DUAS informa��es por posicao(posicao=record com pf+cc)}

procedure ordea(var vet:vet1; t:integer);
{ordea eh um procedimento que ordena um vetor com UMA informa��o por posicao(string[35])}
...
{validacpf eh uma funcao que calcula os digitos verificadores de um cpf e devolve true se digitos calculados iguais ao digitos
 lidos (�ndices 10 e 11). Transforma o string[11] em um vetor integer de 11 posi�oes}
...
PROCEDURE pebin1(vet:vetor;ext:tipo;t:integer;VAR posi:INTEGER);//vet, ext, t por valor e posi por referencia, parametros formais
														 ...
														 // vet possui duas informa��es por posicao o campo chave e a posicao fisica
...
begin
 tot:=t;
 jor:=1;  //variavel que conterah o total de elementos jah ordenados, come�a com 1 (danca youtub, viu????)
 repeat
  tro:=true;
  tot:=tot-jor;
  for a:=1 to tot do
...
begin
 ...
 if(a=11)then                    //fim digitos iguais 
  aux:=false //<<<<<<<<<<<<<<<<<<<ATEN��O PARA FUNCIONAR MUDAR =true PARA =false
 else 
...
function poti(x,y:integer):integer;// x,y por valor, parametros formais
                                   //o resultado da tarefa eh devolvido no nome da funcao
var pot,a:integer;//vari�vel local
 begin
  pot:=1;
  if(y>0)then 
    for a:=1 to y do
